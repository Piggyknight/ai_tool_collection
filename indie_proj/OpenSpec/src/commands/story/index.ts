...
async function refineStory(storyName: string, options: { addAcceptanceCriteria?: boolean }) {
  ...
  if (storyContent.includes('## Acceptance Criteria')) {
    ...
    if (!options.addAcceptanceCriteria) {
      const action = await select({
        ...
        choices: [
          { name: 'Add more criteria', value: 'Add more criteria' },
          { name: 'View current criteria', value: 'View current criteria' },
          { name: 'Edit criteria', value: 'Edit criteria' },
  ...
  const newCriteria = await editor({
